...
def test_category_bubble_shooters():
    ...
    #клик по кнопке с категорией
    button1 = browser.find_element(By.XPATH, "//*[text()='Bubble shooters']")
    ...
    assert "Bubble shooters" in title.text, "Переход не выполнен, либо заголовок не найден"
    time.sleep(2)

    browser.quit()

#test_category_bubble_shooters()
