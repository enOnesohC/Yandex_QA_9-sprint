...
def test_tags_withplayersratings():
    ...
    button1 = browser.find_element(By.XPATH, "//*[text()='with player ratings']")
    ...
    assert "with player ratings" in title.text, "Переход не выполнен, либо заголовок не найден"
    time.sleep(2)

    browser.quit()

#test_tags_withplayersratings()
